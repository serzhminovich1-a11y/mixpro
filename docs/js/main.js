...
// Контурные SVG-иконки статуса урока вместо эмодзи
const ICON_LESSON_DONE='<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><path d="M20 6 9 17l-5-5"/></svg>';
const ICON_LESSON_PLAY='<svg viewBox="0 0 24 24" fill="currentColor"><path d="M8 5v14l11-7z"/></svg>';
const ICON_LESSON_LOCK='<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><rect width="18" height="11" x="3" y="11" rx="2" ry="2"/><path d="M7 11V7a5 5 0 0 1 10 0v4"/></svg>';
...
function renderLessons(){
  ...
  LESSONS.forEach((l,i)=>{
    ...
    card.innerHTML=(isDone?'<div class="l-done-bar"></div>':'')+
      '<div class="lesson-head"><span class="l-num">'+l.num+'</span><span class="l-title">'+l.title+'</span><span class="l-icon">'+(isDone?ICON_LESSON_DONE:u?ICON_LESSON_PLAY:ICON_LESSON_LOCK)+'</span></div>'+
      '<div class="l-desc">'+l.desc+'</div>'+
      (u&&l.body?'<div class="lesson-body" id="lb-'+i+'">'+l.body+'<div style="margin-top:18px"><button onclick="markDone('+i+',event)" style="font-family:var(--display);font-size:13px;font-weight:600;padding:9px 20px;border-radius:7px;border:1px solid var(--green);background:rgba(61,220,132,.08);color:var(--green);cursor:pointer;display:inline-flex;align-items:center;gap:6px">'+(isDone?ICON_LESSON_DONE+'Пройден':'Отметить пройденным')+'</button></div></div>':'');
